# v0.4 Roadmap: Agent-first workflows for low-friction research usage
...
`research-utils` v0.4 focuses on making agent-assisted usage a first-class, reproducible,
...
1. adapt an existing simulation codebase to work with `research-utils` contracts,
...
v0.4 remains aligned with repository boundaries:
...
All v0.4 work should satisfy these constraints:
...
Each v0.4 milestone should include the following verification layers:
...
1. A lab member can obtain deterministic adaptation guidance for integrating an existing
   simulation workflow with `research-utils`.
