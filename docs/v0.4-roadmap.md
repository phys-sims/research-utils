...
`sim-utils` v0.4 focuses on making agent-assisted usage a first-class, reproducible,
...
1. adapt an existing simulation codebase to work with `sim-utils` contracts,
...
1. A lab member can obtain deterministic adaptation guidance for integrating an existing
   simulation workflow with `sim-utils`.
